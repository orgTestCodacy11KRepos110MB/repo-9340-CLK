	...
	switch(address&3) {
		default: {
			...
			switch(status_.type) {
				case Status::One:
					status |=
						...
						(drive().is_read_only() ? Flag::WriteProtect : 0) |
						(drive().index_pulse() ? Flag::Index : 0);
				break;
			...
			if(!has_motor_on_line()) {
				status |= drive().is_ready() ? 0 : Flag::NotReady;
				if(status_.type == Status::One)
					status |= (head_is_loaded_ ? Flag::HeadLoaded : 0);
			} else {
				status |= (drive().motor_on() ? Flag::MotorOn : 0);
				if(status_.type == Status::One)
...
void WD1770::posit_event(int new_event_type) {
	...
	*/
	...
	begin_type1_spin_up:
		if((command_&0x08) || drive().motor_on()) {
			set_motor_on(true);
			goto test_type1_type;
		}
		SPIN_UP();
	...
	perform_step:
		if(!step_direction_ && drive().is_track_zero()) {
			track_ = 0;
			goto verify_seek;
		}
		drive().step(Storage::Disk::HeadPosition(step_direction_ ? 1 : -1));
	...
	verify_seek:
		update_status([this] (Status &status) {
			status.track_zero = drive().is_track_zero();
		});
		if(!(command_ & 0x04)) {
	...
	*/
	...
	begin_type2_spin_up:
		if(drive().motor_on()) goto test_type2_delay;
		// Perform spin up.
		SPIN_UP();
	...
	test_type2_write_protection:
		if(command_&0x20 && drive().is_read_only()) {
			update_status([] (Status &status) {
	...
	*/
	...
	begin_type3_spin_up:
		if((command_&0x08) || drive().motor_on()) goto type3_test_delay;
		SPIN_UP();
